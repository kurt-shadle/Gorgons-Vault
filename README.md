...
- **Trip plan** — Plan a multi-stop route: pick maps in order and see what to pick up and drop at each stop to consolidate stacks. Map options include all CDN cities plus **On character** (items on your character) and **Saddlebag**. Add or remove individual stops; clear route resets to two empty stops. Results are grouped by stop, then by storage/NPC within each stop so you can visit each vault once. A summary line at the top shows *"You could save X slot(s) total by consolidating the items below"* for the planned route.
